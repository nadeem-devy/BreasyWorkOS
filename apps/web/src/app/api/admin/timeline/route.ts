...
export async function GET(request: NextRequest) {
  ...
  const applyUserFilter = (query: ReturnType<ReturnType<typeof supabase.from>['select']>) => {
    if (userId && userId !== 'all') return query.eq('user_id', userId);
    return query;
  };

  const [bubble, financial, gmail, dialpad, melio, activity] = await Promise.all([
    applyUserFilter(
      supabase.from('OS_bubble_events').select('*').gte('created_at', startOfDay).lte('created_at', endOfDay)
    ).order('created_at'),
    applyUserFilter(
      supabase.from('OS_bubble_financial_events').select('*').gte('created_at', startOfDay).lte('created_at', endOfDay)
    ).order('created_at'),
    applyUserFilter(
      supabase.from('OS_gmail_events').select('*').gte('created_at', startOfDay).lte('created_at', endOfDay)
    ).order('created_at'),
    applyUserFilter(
      supabase.from('OS_dialpad_events').select('*').gte('started_at', startOfDay).lte('started_at', endOfDay)
    ).order('started_at'),
    applyUserFilter(
      supabase.from('OS_melio_events').select('*').gte('created_at', startOfDay).lte('created_at', endOfDay)
    ).order('created_at'),
    applyUserFilter(
      supabase.from('OS_activity_events').select('*').gte('created_at', startOfDay).lte('created_at', endOfDay).in('event_type', ['tab_activated', 'idle_start', 'idle_end'])
    ).order('created_at'),
  ]);
  ...
  let sessionsQuery = supabase
    .from('OS_sessions')
    .select('time_bubble, time_gmail, time_dialpad, time_melio, time_idle, time_other, duration_seconds, user_id')
    .gte('started_at', startOfDay)
    .lte('started_at', endOfDay);
  if (userId && userId !== 'all') sessionsQuery = sessionsQuery.eq('user_id', userId);
